(async () => {
  ...
  const createUI = () => {
    ...
    panel.innerHTML = `
      ...
      <div class="wplace-content">
        ...
        <div class="wplace-controls" style="margin-top: 15px;">
          <button id="saveBtn" class="wplace-btn wplace-btn-primary" title="${state.language === 'pt' ? 'Salvar configuração' : 'Save configuration'}">
            <i class="fas fa-save"></i>
            <span>${state.language === 'pt' ? 'Save' : 'Save'}</span>
          </button>
          <button id="loadBtn" class="wplace-btn wplace-btn-primary" title="${state.language === 'pt' ? 'Carregar configuração' : 'Load configuration'}">
            <i class="fas fa-folder-open"></i>
            <span>${state.language === 'pt' ? 'Load' : 'Load'}</span>
          </button>
        </div>
        
        <div class="wplace-controls" style="margin-top: 10px;">
          <button id="exportBtn" class="wplace-btn wplace-btn-primary" title="${state.language === 'pt' ? 'Exportar estatísticas' : 'Export statistics'}">
    ...
    // Thêm event listeners cho các nút Save/Load/Export
  ...
  // Thêm chức năng Save/Load configuration
  const saveConfiguration = () => {
    ...
    link.href = url;
    link.download = `wplace-config-${new Date().toISOString().slice(0,19).replace(/[:.]/g, '-')}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    
    updateUI(state.language === 'pt' ? '💾 Configuração salva!' : '💾 Configuration saved!', 'success');
  };
    ...
    input.onchange = (event) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (configData.state) {
            // Cập nhật state
            state.paintedCount = configData.state.paintedCount || 0;
            state.language = configData.state.language || 'en';
            state.autoRefresh = configData.state.autoRefresh !== undefined ? configData.state.autoRefresh : true;
            state.lastPixel = configData.state.lastPixel || null;
          ...
          updateUI(state.language === 'pt' ? '📁 Configuração carregada!' : '📁 Configuration loaded!', 'success');
          updateStats();
          
        } catch (error) {
          updateUI(state.language === 'pt' ? '❌ Erro ao carregar arquivo' : '❌ Error loading file', 'error');
  ...
  const exportStats = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `wplace-stats-${new Date().toISOString().slice(0,19).replace(/[:.]/g, '-')}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
